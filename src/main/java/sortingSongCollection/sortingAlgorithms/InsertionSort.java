...
public class InsertionSort implements SortingAlgorithm {
    ...
    public void sort(Song[] songs, int low, int high, boolean isAscending, StringBuilder sb) {
            ...
            for (int i = low+1; i <= high; i++) {
                curr = songs[i];      // insert curr into the sorted part of the list
                j = i - 1;  // last element in the sorted part
                while ((j >= low) && (isAscending &&  curr.compareTo(songs[j]) < 0 || (!isAscending && curr.compareTo(songs[j]) > 0)) ){
                    songs[j + 1] = songs[j];
                    j--;
                }
                songs[j + 1] = curr;
